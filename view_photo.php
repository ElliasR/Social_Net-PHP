...
<?php
$get_photos=mysqli_query($con,"SELECT * FROM photos WHERE uid='$picture' && username='$username' && removed='no' ORDER BY id DESC");
...
while($row=mysqli_fetch_array($get_photos)){
	...
	if(isset($_POST['confirmation_remove_photo_' . $id . ''])){
		$remove_album=mysqli_query($con,"UPDATE photos SET removed='yes' WHERE id='$id'");
	...
	$nmodel = mysqli_real_escape_string($con,$filtrado);
	...
	$ndesc = mysqli_real_escape_string($con,$filtrado);		
		...
		$update_pic=mysqli_query($con,"UPDATE photos SET title='$ntitle', tags='$ntag', modelname='$nmodel', description='$ndesc' WHERE id='$id'");
		header("Location:view_albums.php");
	}
	...
	<td>
		<center>
			...
			<div class='albums'>	
				...
				$sortdesc
				<a href='rate_body.php?indiv=$indiv'><i class='fa fa-arrows-alt fa-lg' aria-hidden='true' style='color:white;float:right;'></i></a>
			</div>
			<a onclick=\"document.getElementById('conten$id').style.display=(document.getElementById('conten$id').style.display=='none')?'block':'none';\"><h2 class='hiding' style='max-width:200px;'>Modify data</h2></a></br>
				<form id='conten$id' style='display:none;' method='POST' action=''>
					...
					<input type='text' name='model' id='model' size='40' placeholder='Name of the model: $modelname '></br>
