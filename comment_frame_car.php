...
if (isset($_POST['postcomment' . $getid . ''])){
	...
	$insertpost= mysqli_query($con,"INSERT INTO post_comments_car VALUES ('','$post_body','$username','$posted_to','0','$getid','$date')");
}
?>
<a href='javascript:;' onClick="javascript:toggle()"><div style='float:right; display:inline;'>Post a comment</div></a>
	<div id='toggleComment' style='display:none;'>
		<form action="comment_frame_car.php?id=<?php echo htmlspecialchars($getid);?>" method="POST" name="postcomment<?php echo htmlspecialchars($getid);?>">
...
$get_comments=mysqli_query($con,"SELECT * FROM post_comments_car WHERE post_id='$getid' ORDER BY id ASC");
